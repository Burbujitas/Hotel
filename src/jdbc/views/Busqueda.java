...
public class Busqueda extends JFrame {
	...
	private DefaultTableModel modeloHabitacion;
	...
	public Busqueda() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(Busqueda.class.getResource("/jdbc/imagenes/lupa2.png")));
		...
		panel.addTab("Huéspedes", new ImageIcon(Busqueda.class.getResource("/jdbc/imagenes/pessoas.png")), tbHuespedes, null);
		...
		panel.addTab("Reservas", new ImageIcon(Busqueda.class.getResource("/jdbc/imagenes/reservado.png")), tbReservas, null);
		...
		tbHabitaciones.setFont(new Font("Roboto", Font.PLAIN, 16));
		panel.addTab("Habitaciones", new ImageIcon(Busqueda.class.getResource("/jdbc/imagenes/reservado.png")), tbHabitaciones, null);
		modeloHabitacion = (DefaultTableModel) tbHabitaciones.getModel();
		modeloHabitacion.addColumn("Numero de Habitacion");
		modeloHabitacion.addColumn("nombre");
		modeloHabitacion.addColumn("Habitaciones");
		modeloHabitacion.addColumn("direccion");
		tbHabitaciones.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);

		LlenarTablaHabitacion();

		JLabel logo = new JLabel("");
		logo.setBounds(56, 51, 104, 107);
		logo.setIcon(new ImageIcon(Busqueda.class.getResource("/jdbc/imagenes/Ha-100px.png")));
		...
		btnEditar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int filaHabitaciones = tbHabitaciones.getSelectedRow();
				if (filaReservas >= 0) {
					ActualizarReservas();
					limpiarTabla();
					LlenarTablaReservas();
		...
		btnEliminar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (filaReservas >= 0) {
					...
					habitaciones = tbHabitaciones.getValueAt(filaHabitaciones, 0).toString();
					int confirmarh = JOptionPane.showConfirmDialog(null, "¿Desea Eliminar los datos?");

					if(confirmarh == JOptionPane.YES_OPTION){

						String valor = tbHabitaciones.getValueAt(filaHabitaciones, 0).toString();
						habitacionController.Eliminar(Integer.valueOf(valor));
						JOptionPane.showMessageDialog(contentPane, "Registro Eliminado");
						limpiarTabla();
						LlenarTablaHuespedes();
						LlenarTablaReservas();
	...
	private List<Habitacion> BuscarHabitaciones()
	{
		return this.habitacionController.listarHabitaciones();
	}
	private List<Habitacion> BuscarHabitacionesId()
	{
		return this.habitacionController.listarHabitacionesId(txtBuscar.getText());
	}
	...
	private void limpiarTabla() {
		...
		((DefaultTableModel) tbHabitaciones.getModel()).setRowCount(0);
	}
	...
	{
		List<Habitacion> habitaciones = BuscarHabitaciones();
		try {
			for(Habitacion habitacion : habitaciones){
				modeloHabitacion.addRow(new Object[] { habitacion.getId(), habitacion.getNombre(),habitacion.getHabitaciones(),habitacion.getDireccion()});
			}
		} catch (Exception e){
			throw e;
		}
	}
	private void LlenarTablaHabitacionId()
	{
		List<Habitacion> habitaciones = BuscarHabitacionesId();
		try {
			for(Habitacion habitacion : habitaciones){
				modeloHabitacion.addRow(new Object[] { habitacion.getId(), habitacion.getNombre(),habitacion.getHabitaciones(),habitacion.getDireccion()});
			}
	...
	{
		Optional.ofNullable(modeloHabitacion.getValueAt(tbHabitaciones.getSelectedRow(),tbHabitaciones.getSelectedColumn()))
				.ifPresentOrElse(filaHabitacion -> {
					String nombre = (String) modeloHabitacion.getValueAt(tbHabitaciones.getSelectedRow(),1);
					Integer habitaciones = Integer.valueOf(modeloHabitacion.getValueAt(tbHabitaciones.getSelectedRow(),2).toString());
					String direccion = (String) modeloHabitacion.getValueAt(tbHabitaciones.getSelectedRow(),3);
					Integer id = Integer.valueOf(modeloHabitacion.getValueAt(tbHabitaciones.getSelectedRow(),0).toString());
					this.habitacionController.actualizar(nombre,habitaciones,direccion,id);
					JOptionPane.showMessageDialog(this,String.format("Registro modificado con éxito"));
